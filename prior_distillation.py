...
class TrainOptions():
    def __init__(self):
        ...
        self.parser.add_argument("--batch", type=int, default=10, help="batch size")
        self.parser.add_argument("--lambda_app_rec", type=float, default=1.0,
                                 help="the weight of appearance distant loss")
        self.parser.add_argument("--lambda_shape_rec", type=float, default=5.0,
                                 help="the weight of shape reconstruction loss")
        ...
        self.parser.add_argument("--lambda_feat_dist", type=float, default=1.0,
                                 help="the weight of feature distant loss")
        self.parser.add_argument("--lambda_shape_dist", type=float, default=5.0,
        ...
        self.parser.add_argument("--unpaired_mask_root", type=str,
                                 help="the path to the ImageNet291_mask and CelebA-HQ_mask")
...
def train(args, udataloader, pdataloader, netAE, optimizer_AE, vgg_loss, device='cuda'):
    ...
    for idx in pbar:
        ...
        la, xa, lb, xb, ma, mb = pdata['label'], pdata['image'], pdata['labelB'], pdata['imageB'], pdata['mask'], pdata[
            'maskB']
        ...
        Lfdist = F.l1_loss(content_feat[args.batch:args.batch * 2],
                           content_feat[args.batch * 2:args.batch * 3].detach()) * args.lambda_feat_dist
        ...
        if idx == 0 or (idx + 1) % args.visualize_every == 0 or (idx + 1) == args.iter:
            ...
            sample = F.adaptive_avg_pool2d(torch.cat((imgs[0:viznum], masks[0:viznum],
                                                      ...
                                                      imgs[args.batch:args.batch + viznum],
                                                      masks[args.batch:args.batch + viznum],
                                                      recon_imgs[args.batch:args.batch + viznum],
                                                      recon_masks[args.batch:args.batch + viznum],
                                                      imgs[args.batch * 2:args.batch * 2 + viznum],
                                                      masks[args.batch * 2:args.batch * 2 + viznum],
                                                      trans_masks[0:viznum],
                                                      trans_masks[args.batch:args.batch + viznum]), dim=0), 128).cpu()
            utils.save_image(
                sample,
            ...
            # test_tensorboard
            writer.add_scalar('Larec', Larec, global_step=idx)
            writer.add_scalar('Lsrec', Lsrec, global_step=idx)
            writer.add_scalar('Lfdist', Lfdist, global_step=idx)
            writer.add_scalar('Lsdist', Lsdist, global_step=idx)
            writer.add_scalar('Lreg', Lreg, global_step=idx)
            #
...
if __name__ == "__main__":
    ...
    udataloader = create_imagemasklabel_dataloader(args.unpaired_data_root, args.unpaired_mask_root,
                                                   ufiles, udataset_sizes, ulabels, batchSize=args.batch)

    # for paired data
    pdataloader = create_imagemasklabel_dataloader(args.paired_data_root, args.paired_mask_root,
                                                   pfiles, pdataset_sizes, plabels, batchSize=args.batch, pair=True)
