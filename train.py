...
class TrainOptions():
    def __init__(self):
        ...
        self.parser.add_argument("--iter", type=int, default=180000, help="iterations")
        ...
        self.parser.add_argument("--lambda_styr", type=float, default=0.5, help="the weight of styr loss")
...
def train(args, dataloader, target_dataloader, netG, netD, optimizer_G, optimizer_D, netG_ema,
    ...
    for idx in pbar:
        ...
        ybar_sty = netG.style_encoder(ybar)
        yreal_sty = netG.style_encoder(y_)
        Lstyr = F.mse_loss(ybar_sty, yreal_sty)*args.lambda_styr
        ...
        loss_dict['msk'] = Lmsk
        loss_dict['rec'] = Lrec
        loss_dict['styr']=Lstyr
        if args.use_idloss:
            loss_dict['id'] = Lid

        g_loss = Lgadv + Lcon + Lsty + Lmsk + Lrec + Lid +Lstyr
        ...
        if idx == 0 or (idx + 1) % args.visualize_every == 0 or (idx + 1) == args.iter:
            ...
            # writer.add_scalar('Lgadv', Lgadv.item(), global_step=idx)
            # writer.add_scalar('Lcon', Lcon.item(), global_step=idx)
            # writer.add_scalar('Lsty', Lsty.item(), global_step=idx)
            # writer.add_scalar('Lmsk', Lmsk.item(), global_step=idx)
            # writer.add_scalar('Lrec', Lrec.item(), global_step=idx)
            # writer.add_scalar('Lid', Lid.item(), global_step=idx)
            # writer.add_scalar('Ldadv', Ldadv.item(), global_step=idx)
            # writer.add_scalar('Lr1', Lr1.item(), global_step=idx)
            # writer.add_scalar('Lstyr', Lstyr.item(), global_step=idx)
            # d_loss = Ldadv + Lr1
